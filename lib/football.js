...
/**
 * Lấy diễn biến THẬT (cầu thủ ghi bàn, phút, thẻ phạt) của một trận từ
 * football-data.org — KHÔNG tốn quota AI. Trả { events:[...] } hoặc null
 * nếu không có token / không phải trận từ football-data / API không trả về.
 * Định dạng event giống Gemini: { minute, team:'home'|'away', player, type }.
 */
export async function fetchMatchEventsFromFD(match) {
  if (!process.env.FOOTBALL_DATA_TOKEN) return null;
  const id = String(match?.id || '');
  if (!id.startsWith('FD-')) return null;          // trận từ nguồn AI thì bỏ qua

  const res = await fetch(`https://api.football-data.org/v4/matches/${id.slice(3)}`, {
    headers: { 'X-Auth-Token': process.env.FOOTBALL_DATA_TOKEN }
  });
  if (!res.ok) {
    console.error('[fd-match]', res.status, (await res.text()).slice(0, 160));
    return null;
  }
  const m = await res.json();
  const homeId = m.homeTeam?.id;
  const side = teamId => (teamId === homeId ? 'home' : 'away');
  const min = ev => (ev.minute != null ? ev.minute + (ev.injuryTime ? ev.injuryTime : 0) : null);

  const events = [];
  for (const g of (m.goals || [])) {
    const type = g.type === 'OWN' ? 'own_goal' : g.type === 'PENALTY' ? 'penalty' : 'goal';
    events.push({ minute: min(g), team: side(g.team?.id), player: g.scorer?.name || '', type });
  }
  for (const b of (m.bookings || [])) {
    const type = (b.card === 'RED' || b.card === 'YELLOW_RED') ? 'red' : 'yellow';
    events.push({ minute: min(b), team: side(b.team?.id), player: b.player?.name || '', type });
  }
