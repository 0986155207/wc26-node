    ...
    _schemaReady = (async () => {
      ...
      await q`
        CREATE TABLE IF NOT EXISTS push_subs (
          ...
          sub         JSONB NOT NULL,
          prefs       JSONB NOT NULL DEFAULT '{"daily":true,"kickoff":true,"result":true}'::jsonb,
          created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
        )`;
      await q`
      ...
      // Bổ sung cột loạt sút luân lưu (cho vòng knock-out) — an toàn nếu đã tồn tại
      await q`ALTER TABLE matches ADD COLUMN IF NOT EXISTS home_pen INTEGER`;
      await q`ALTER TABLE matches ADD COLUMN IF NOT EXISTS away_pen INTEGER`;
      // Quốc gia tổ chức & sức chứa sân (bổ sung từ ESPN)
      await q`ALTER TABLE matches ADD COLUMN IF NOT EXISTS country TEXT NOT NULL DEFAULT ''`;
      await q`ALTER TABLE matches ADD COLUMN IF NOT EXISTS capacity INTEGER`;
    })();
  }
  return _schemaReady;
}
