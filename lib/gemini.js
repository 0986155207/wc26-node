...
import { fetchMatchEventsFromTSDB } from './football.js';
...
export async function getMatchDetails(matchId) {
  ...
  // 1) Ưu tiên diễn biến THẬT từ TheSportsDB (không tốn quota AI)
  let events = [];
  try {
    const tsdb = await fetchMatchEventsFromTSDB(m);
    if (tsdb && tsdb.events.length) events = tsdb.events;
  } catch (e) {
    console.warn('[match-details] TheSportsDB lỗi:', e.message);
  }
