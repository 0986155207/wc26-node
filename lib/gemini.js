...
export async function getMatchDetails(matchId) {
  ...
  const goalCount = arr => (arr || []).filter(e => ['goal', 'penalty', 'own_goal'].includes(e.type)).length;
  const totalGoals = (Number(m.home_goals) || 0) + (Number(m.away_goals) || 0);
  // "Đủ" = số bàn có người ghi ≥ tỷ số (trận 0-0 thì hiển nhiên đủ).
  const isEnough = arr => goalCount(arr) >= totalGoals;

  // Kiểm tra cache — chỉ dùng lại khi dữ liệu ĐÃ ĐỦ người ghi bàn; cache cũ còn
  // thiếu (vd nguồn trước chỉ có 1/2 bàn) sẽ được làm mới để tự "lành".
  ...
  if (cached.length) {
    ...
    const cacheOk = done ? isEnough(cached[0].events) : ageMs < 60_000;
    if (cacheOk) {
      return {
        matchId,
  ...
  //    ESPN → TheSportsDB. Dừng ngay khi đã đủ người ghi bàn (hoặc trận 0-0);
  //    nếu chưa nguồn nào đủ, giữ bộ có nhiều bàn/sự kiện nhất.
  let events = [];
  for (const [name, fetcher] of [['ESPN', fetchMatchEventsFromESPN], ['TheSportsDB', fetchMatchEventsFromTSDB]]) {
    try {
      ...
      if (r && r.events.length) {
        if (isEnough(r.events)) { events = r.events; break; }
        if (goalCount(r.events) > goalCount(events) || r.events.length > events.length) events = r.events;
      }
    } catch (e) {
  ...
  // 2) Tóm tắt. Đủ người ghi bàn → dựng ngay, khỏi cần AI. Còn thiếu → nhờ Gemini
  //    tra cứu, nhưng vẫn giữ sự kiện đã có (vd thẻ phạt) nếu Gemini không trả được.
  let summary;
  if (events.length && isEnough(events)) {
    summary = autoSummary(m, events);
  } else {
    try {
      const r = await geminiMatchSearch(m);
      if (r.events && goalCount(r.events) >= goalCount(events)) events = r.events;
  ...
  // Chỉ cache khi đã đủ người ghi bàn (tránh "khóa" lúc còn thiếu để lần sau còn
  // thử lại khi nguồn dữ liệu / quota hồi phục).
  const complete = isEnough(events);
  if (complete) {
    await q`
