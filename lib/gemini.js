...
import { fetchMatchEventsFromFD } from './football.js';
...
function autoSummary(m, events) {
  const score = `${m.home} ${m.home_goals ?? '?'}-${m.away_goals ?? '?'} ${m.away}`;
  const goals = (events || []).filter(e => ['goal', 'penalty', 'own_goal'].includes(e.type));
  if (!goals.length) {
    return m.status === 'Kết thúc'
      ? `${score}. Chưa có dữ liệu chi tiết về người ghi bàn cho trận này.`
      : `${score}. Trận đang diễn ra.`;
  }
  const tag = t => (t === 'penalty' ? ' (pen)' : t === 'own_goal' ? ' (phản lưới)' : '');
  const list = goals.map(g => {
    const team = g.team === 'away' ? m.away : m.home;
    const min = g.minute != null ? ` ${g.minute}'` : '';
    return `${g.player || '?'}${min} — ${team}${tag(g.type)}`;
  }).join('; ');
  return `${score}. Người ghi bàn: ${list}.`;
}
...
export async function getMatchDetails(matchId) {
  ...
  // 1) Ưu tiên diễn biến THẬT từ football-data.org (không tốn quota AI)
  let events = [];
  try {
    const fd = await fetchMatchEventsFromFD(m);
    if (fd && fd.events.length) events = fd.events;
  } catch (e) {
    console.warn('[match-details] football-data lỗi:', e.message);
  }

  // 2) Tóm tắt: có sự kiện thật → dựng ngay; nếu không → nhờ Gemini tra cứu
  let summary;
  let complete = true;        // chỉ cache khi dữ liệu đáng tin (tránh "khóa" lỗi tạm thời)
  if (events.length) {
    ...
    try {
      ...
      // Gemini quá tải → vẫn trả tỷ số + thông báo nhẹ nhàng, KHÔNG cache để
      // lần sau thử lại khi quota hồi phục.
      console.warn('[match-details] Gemini lỗi, dùng tóm tắt tối giản:', e.message);
      summary = autoSummary(m, []);
      complete = false;
    }
  }
